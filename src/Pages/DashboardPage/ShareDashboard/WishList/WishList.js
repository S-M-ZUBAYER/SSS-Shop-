...
const WishList = () => {
    ...
    let wishListProducts = myOrders.filter(order => order?.paid !== true)
        ...
        fetch(`https://cricket-lover-server-site-s-m-zubayer.vercel.app/newBooking/${order?._id}`, {
            ...
            .then(data => {
                if (data?.deletedCount > 0) {
                    wishListProducts = wishListProducts.filter(product => order?._id !== product._id);
                            ...
                            <div className="flex flex-col justify-center items-center lg:flex-row space-y-28 lg:space-y-0  lg:space-x-10 mb-12">
                                <div
                                    ...
                                    <div className="flex w-full justify-between">
                                        ...
                                        <button
                                            onClick={() => handleToDelete(order)}
                                            className="text-center text-sm px-6 bg-red-500 py-2 mt-5 rounded-3xl hover:bg-cyan-500 p-5 text-white font-medium">
                                            Delete
                                        </button>
